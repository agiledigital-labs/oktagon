    ...
    TE.chain(
      flow(
        O.fold(
          ...
          () => service.expirePasswordAndGetTemporaryPassword(userId)
        )
      )
    ),
...
export default (
  ...
  rootCommand.command(
    ...
    'expires the password for a user and gets a temporary password for them, only works if user currently has the status: staged, provisioned, locked out or password expired.',
